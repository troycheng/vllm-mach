"""TP2 M4096 all-reduce/residual/RMSNorm input-and-sum codec boundary screen.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--reference-library",
        required=True,
        help="Frozen input-only mach_lossless_prefill extension .so path.",
    )
    parser.add_argument(
        "--library",
        required=True,
        help="New mach_lossless_prefill_sum extension .so path.",
    )
...
def make_arms(rank: int) -> dict[str, Arm]:
    ...
    return {
        ...
        "champion": Arm(
            ...
            champion_ws,
            streams["champion"],
            extension_dispatch("mach_lossless_prefill", champion_ws),
        ),
        "sum_packed": Arm(
            "sum_packed",
            sum_packed_ws,
            streams["sum_packed"],
            extension_dispatch("mach_lossless_prefill_sum", sum_packed_ws),
        ),
    }
...
def read_codec_info(namespace: str, sample: torch.Tensor) -> Any:
    info = getattr(torch.ops, namespace).info(sample)
...
def main() -> None:
    ...
    try:
        ...
        for namespace in ("mach_lossless_prefill", "mach_lossless_prefill_sum"):
            if (
...
if __name__ == "__main__":
    try:
        ...
        print(f"bench_sum_codec failed: {error}", file=sys.stderr, flush=True)
        traceback.print_exc()
        raise
