...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--reference-library", required=True, help="Frozen input-only mach_lossless_prefill extension .so path.")
    parser.add_argument("--library", required=True, help="New mach_lossless_prefill_sum extension .so path.")
...
def make_arms(rank: int) -> dict[str, Arm]:
    ...
    # across installed, frozen input-only Champion, and new sum-packed paths.
    ...
    def extension_dispatch(namespace: str, workspace: Any) -> Callable[[torch.Tensor, torch.Tensor, torch.Tensor], None]:
        ...
        def dispatch(input_: torch.Tensor, residual_out: torch.Tensor, norm_out: torch.Tensor) -> None:
            getattr(torch.ops, namespace).run(
                ...
                rank, workspace_bytes, 1e-6, 1.0, True, False,
            )
    ...
    return {
        ...
        "champion": Arm("champion", champion_ws, streams["champion"], extension_dispatch("mach_lossless_prefill", champion_ws)),
        "sum_packed": Arm("sum_packed", sum_packed_ws, streams["sum_packed"], extension_dispatch("mach_lossless_prefill_sum", sum_packed_ws)),
    }
...
def main() -> None:
    ...
    try:
        ...
        codec_info = {namespace: read_codec_info(namespace, fixtures[0].source) for namespace in ("mach_lossless_prefill", "mach_lossless_prefill_sum")}
