...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--library",
        required=True,
        help="Exact mach_lossless_prefill_direct extension .so path.",
    )
...
def main() -> None:
    ...
    try:
        ...
        if not hasattr(torch.ops, "mach_lossless_prefill_direct") or not hasattr(
            torch.ops.mach_lossless_prefill_direct, "run"
        ):
            raise RuntimeError(
                "extension did not register torch.ops.mach_lossless_prefill_direct.run"
            )
        reference = WorkspaceArm(
