...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--library", required=True, help="Exact mach_lossless_prefill_sum extension .so path.")
...
def candidate_dispatch(
    ...
    torch.ops.mach_lossless_prefill_sum.run(
        ...
        rank, int(workspace.metadata["buffer_size"]), 1e-6, 1.0, True, True,
    )
...
def main() -> None:
    ...
    try:
        ...
        if not hasattr(torch.ops, "mach_lossless_prefill_sum") or not hasattr(torch.ops.mach_lossless_prefill_sum, "run"):
            raise RuntimeError("extension did not register torch.ops.mach_lossless_prefill_sum.run")
