...
import bench_sum_codec as common
import torch
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--library",
        required=True,
        help="Exact mach_lossless_prefill_sum extension .so path.",
    )
...
def create_workspace(rank: int) -> Any:
    ...
    if (
        ...
        raise RuntimeError(
            "workspace lacks workspace_tensor or metadata['buffer_size']"
        )
    if int(workspace.metadata["buffer_size"]) < common.MIN_WORKSPACE_BYTES:
        raise RuntimeError(
            "large workspace does not meet the M4096 twoshot payload/header minimum"
        )
    return workspace
...
def deterministic_tensor(
    rows: int, seed: int, device: torch.device, scale: float = 0.01
) -> torch.Tensor:
    generator = torch.Generator(device=device).manual_seed(seed)
    return (
        torch.randn(
            (rows, HIDDEN), device=device, dtype=torch.float32, generator=generator
        )
        * scale
    ).to(torch.bfloat16)
...
def make_small_inputs(device: torch.device) -> dict[int, torch.Tensor]:
    return {
        rows: deterministic_tensor(rows, 20261000 + rows, device) for rows in SMALL_ROWS
    }
...
def make_gamma(device: torch.device) -> torch.Tensor:
    ...
    return (
        1.0
        + torch.randn(
            (HIDDEN,), device=device, dtype=torch.float32, generator=generator
        )
        * 0.01
    ).to(torch.bfloat16)
...
def candidate_dispatch(
    ...
    torch.ops.mach_lossless_prefill_sum.run(
        input_,
        residual,
        gamma,
...
def capture_small_graph(
    arm: WorkspaceArm, source: torch.Tensor, residual: torch.Tensor, gamma: torch.Tensor
) -> SmallGraph:
...
def replay_small_graph(
    arm: WorkspaceArm, graph: SmallGraph
) -> tuple[torch.Tensor, torch.Tensor]:
    graph.input.copy_(graph.restore)
...
def compare_pair(
    ...
    common.bitwise_equal(
        reference[0], candidate[0], f"{label}: residual_out/input alias"
    )
...
def collect_and_write(
    ...
    payload = {
        "records": records,
        "workspace": {
            "reference": workspace_metadata(reference.workspace),
            "candidate": workspace_metadata(candidate.workspace),
        },
    ...
    summary = {
        ...
        "candidate": "same installed path for M32/M24; packed mach_lossless_prefill_sum twoshot only for M4096, launch_with_pdl=True",
...
def main() -> None:
    ...
    try:
        ...
        if not hasattr(torch.ops, "mach_lossless_prefill_sum") or not hasattr(
            torch.ops.mach_lossless_prefill_sum, "run"
        ):
            raise RuntimeError(
                "extension did not register torch.ops.mach_lossless_prefill_sum.run"
            )
        reference = WorkspaceArm(
            "reference", create_workspace(rank), torch.cuda.Stream()
        )
        candidate = WorkspaceArm(
            "candidate", create_workspace(rank), torch.cuda.Stream()
        )
        ...
        small_graphs = {
            name: {
                rows: capture_small_graph(
                    arm, small_inputs[rows], small_residuals[rows], gamma
                )
        ...
        fixture_by_round = [
            ...
            next(
                (
                    fixture
                    for fixture in common.synthetic_fixtures(rank, device)
        ...
        for round_index, fixture in enumerate(fixture_by_round):
            ...
            records.append(
                {"round": round_index, "step": "M32_oneshot_graph", "bitwise": True}
            )
            dist.barrier()
            ref_out = run_m4096(
                reference, fixture.source, m4096_residual, gamma, rank, candidate=False
            )
            cand_out = run_m4096(
                candidate, fixture.source, m4096_residual, gamma, rank, candidate=True
            )
            compare_pair(
                ref_out, cand_out, f"round {round_index} M4096 first {fixture.name}"
            )
            records.append(
                {
                    "round": round_index,
                    "step": "M4096_twoshot_first",
            ...
            records.append(
                {"round": round_index, "step": "M24_oneshot_graph", "bitwise": True}
            )
            dist.barrier()
            ref_out = run_m4096(
                reference, fixture.source, m4096_residual, gamma, rank, candidate=False
            ...
            compare_pair(
                ref_out, cand_out, f"round {round_index} M4096 second {fixture.name}"
            )
            records.append(
                {
                    "round": round_index,
                    "step": "M4096_twoshot_second",
                    "fixture": fixture.name,
                    "bitwise": True,
                }
            )
        ...
        collect_and_write(
            ...
            candidate,
            records,
            [fixture.provenance for fixture in fixture_by_round],
        )
        dist.barrier()
        if rank == 0:
