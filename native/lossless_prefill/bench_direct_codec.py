"""TP2 M4096 all-reduce/residual/RMSNorm old-SUM versus direct-SUM screen.
...
MIN_WORKSPACE_BYTES = ROWS * HIDDEN * 4 + ROWS * HIDDEN // 256 * 4
PATTERN = 1
CAPTURE_CALLS = (0, 63, 127)
ARM_NAMES = ("installed", "champion", "sum_packed")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--reference-library",
        required=True,
        help="Frozen input+SUM mach_lossless_prefill_sum extension .so path.",
    )
    parser.add_argument(
        "--library",
        required=True,
        help="New mach_lossless_prefill_direct extension .so path.",
    )
    ...
    parser.add_argument(
        "--validate-only",
        action="store_true",
        help="Run validation and CUDA-Graph replay checks only.",
    )
    parser.add_argument(
        "--smoke",
        action="store_true",
        help="Validate synthetic cases only; skip capture fixtures and perf.",
    )
    parser.add_argument(
        "--banks",
        type=int,
        default=9,
        help="Rotating M4096 input/output banks per arm (minimum 8).",
    )
    parser.add_argument(
        "--samples",
        type=int,
        default=18,
        help="Interleaved ABCCBA repetitions (36 samples per arm at 18).",
    )
...
def bitwise_equal(left: torch.Tensor, right: torch.Tensor, label: str) -> None:
    if not torch.equal(left.view(torch.uint16), right.view(torch.uint16)):
        mismatch = torch.nonzero(
            left.view(torch.uint16) != right.view(torch.uint16), as_tuple=False
        )[0]
...
def init_dist(args: argparse.Namespace) -> tuple[int, int, torch.device]:
    ...
    if world_size != 2:
        raise RuntimeError(
            f"requires torchrun --nproc_per_node=2, got world_size={world_size}"
        )
    if local_rank not in (0, 1):
        raise RuntimeError(
            f"requires local CUDA devices 0 and 1, got LOCAL_RANK={local_rank}"
        )
...
def load_capture_fixture(
    capture_dir: Path, rank: int, call: int, device: torch.device
) -> Fixture:
    ...
    if source_shape != (ROWS, HIDDEN):
        raise ValueError(
            f"{meta_path}: source_shape must be {(ROWS, HIDDEN)}, got {source_shape}"
        )
    raw = np.fromfile(data_path, dtype="<u2")
    if raw.size != SAMPLE_ROWS * HIDDEN:
        raise ValueError(
            f"{data_path}: expected {SAMPLE_ROWS * HIDDEN} u16 values, got {raw.size}"
        )
    sample_bits = torch.from_numpy(raw.astype(np.uint16, copy=True)).reshape(
        SAMPLE_ROWS, HIDDEN
    )
    source = (
        sample_bits.view(torch.bfloat16)
        .repeat((REPEAT_ROWS, 1))
...
def synthetic_fixtures(rank: int, device: torch.device) -> list[Fixture]:
    zero = torch.zeros((ROWS, HIDDEN), dtype=torch.bfloat16, device=device)
    signed_zero = torch.full(
        (ROWS, HIDDEN), float("-0.0"), dtype=torch.bfloat16, device=device
    )
    generator = torch.Generator(device=device).manual_seed(20260907)
    normal = (
        torch.randn(
            (ROWS, HIDDEN), generator=generator, device=device, dtype=torch.float32
        )
        * 0.01
    ).to(torch.bfloat16)
    cancellation_value = 1.0 if rank == 0 else -1.0
    cancelling = torch.full(
        (ROWS, HIDDEN), cancellation_value, dtype=torch.bfloat16, device=device
    )
    exponents = torch.arange(10, 61, dtype=torch.int16).repeat((256 + 50) // 51)[:256]
    fractions = torch.arange(256, dtype=torch.int16) & 127
    wide_bits = (exponents << 7 | fractions).to(torch.uint16).reshape(1, 256)
    wide_row = wide_bits.repeat(1, HIDDEN // 256).view(torch.bfloat16)
    wide_exponent = wide_row.repeat((ROWS, 1)).to(device=device, non_blocking=False)
    return [
        Fixture("zero", zero, {"kind": "synthetic", "bits": "0x0000"}),
        Fixture("signedzero", signed_zero, {"kind": "synthetic", "bits": "0x8000"}),
        Fixture(
            "normal", normal, {"kind": "synthetic", "seed": 20260907, "scale": 0.01}
        ),
        Fixture(
            "compact_input_cancelling_sum",
            cancelling,
            {
                "kind": "finite_sum_edge",
                "rank": rank,
                "input": cancellation_value,
                "input_exponent_span": 0,
                "sum_expectation": "exact zero; exercises sum raw fallback",
            },
        ),
        Fixture(
            "wide_exponent_finite",
            wide_exponent,
            {
                "kind": "finite_sum_edge",
                "rank": rank,
                "bf16_exponent_range": [10, 60],
                "span": 50,
                "no_nan_or_inf_input": True,
            },
        ),
    ]
...
def make_residual_and_gamma(device: torch.device) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    gamma = (
        1.0
        + torch.randn(
            (HIDDEN,), generator=gamma_generator, device=device, dtype=torch.float32
        )
        * 0.01
    ).to(torch.bfloat16)
...
def create_workspace(rank: int) -> Any:
    ...
    if getattr(workspace, "backend", None) != "trtllm":
        raise RuntimeError(
            f"expected trtllm workspace, got {getattr(workspace, 'backend', None)!r}"
        )
    if (
        not hasattr(workspace, "workspace_tensor")
        or "buffer_size" not in workspace.metadata
    ):
        raise RuntimeError(
            "FlashInfer workspace does not expose workspace_tensor and metadata['buffer_size']"
        )
...
def make_arms(rank: int) -> dict[str, Arm]:
    ...
    installed_ws, champion_ws, sum_packed_ws = (
        create_workspace(rank) for _ in ARM_NAMES
    )
    streams = {name: torch.cuda.Stream() for name in ARM_NAMES}

    def installed(
        input_: torch.Tensor, residual_out: torch.Tensor, norm_out: torch.Tensor
    ) -> None:
    ...
    def extension_dispatch(
        namespace: str, workspace: Any
        ...
        def dispatch(
            ...
            getattr(torch.ops, namespace).run(
                input_,
                RESIDUAL,
                GAMMA,
                ...
                1e-06,
                1.0,
                1 if namespace == "mach_lossless_prefill_direct" else True,
                False,
            )
    ...
    return {
        ...
        "champion": Arm(
            ...
            champion_ws,
            streams["champion"],
            extension_dispatch("mach_lossless_prefill_sum", champion_ws),
        ),
        "sum_packed": Arm(
            "sum_packed",
            sum_packed_ws,
            streams["sum_packed"],
            extension_dispatch("mach_lossless_prefill_direct", sum_packed_ws),
        ),
    }
...
def run_eager_service_alias(
    arm: Arm, source: torch.Tensor
...
def validate_fixtures(
    arms: dict[str, Arm], fixtures: list[Fixture], rank: int
) -> list[dict[str, Any]]:
    records: list[dict[str, Any]] = []
    for fixture in fixtures:
        dist.barrier()
        outputs = {name: run_eager(arm, fixture.source) for name, arm in arms.items()}
        for name in ("champion", "sum_packed"):
            bitwise_equal(
                outputs["installed"][0],
                outputs[name][0],
                f"{fixture.name} residual installed vs {name}",
            )
            bitwise_equal(
                outputs["installed"][1],
                outputs[name][1],
                f"{fixture.name} norm installed vs {name}",
            )
        records.append(
            {
        ...
        if rank == 0:
            print(
                f"validated {fixture.name}: installed == champion == sum_packed",
                flush=True,
            )
    return records


def validate_service_alias(
    arms: dict[str, Arm], fixtures: list[Fixture], rank: int
) -> dict[str, Any]:
    capture = next(
        (fixture for fixture in fixtures if fixture.name.startswith("capture_call")),
        None,
    )
    if capture is None:
    ...
    dist.barrier()
    outputs = {
        name: run_eager_service_alias(arm, capture.source) for name, arm in arms.items()
    }
    for name in ("champion", "sum_packed"):
        bitwise_equal(
            outputs["installed"][0],
            outputs[name][0],
            f"service-alias residual installed vs {name}",
        )
        bitwise_equal(
            outputs["installed"][1],
            outputs[name][1],
            f"service-alias norm installed vs {name}",
        )
    if rank == 0:
        print(
            "validated service alias: residual_out is allreduce input for real capture",
            flush=True,
        )
    return {
...
def replay_changed_input_graphs(
    arms: dict[str, Arm], fixtures_by_name: dict[str, Fixture], rank: int
) -> list[dict[str, Any]]:
    original_name = next(
        (name for name in fixtures_by_name if name.startswith("capture_call")), "normal"
    )
    selected = ["zero", original_name]
    graphs = {
        name: capture_graph(
            arms[name], fixtures_by_name[original_name].source, original_name
        )
    ...
    for fixture_name in selected:
        ...
        for name in ("champion", "sum_packed"):
            bitwise_equal(
                graphs["installed"].residual_out,
                graphs[name].residual_out,
                f"graph {fixture_name} residual installed vs {name}",
            )
            bitwise_equal(
                graphs["installed"].norm_out,
                graphs[name].norm_out,
                f"graph {fixture_name} norm installed vs {name}",
            )
        records.append(
            {
        ...
        if rank == 0:
            print(
                f"graph replay validated {fixture_name}: installed == champion == sum_packed",
                flush=True,
            )
    return records


def build_perf_banks(
    arms: dict[str, Arm], fixtures: list[Fixture], banks: int
) -> dict[str, list[GraphBank]]:
    if banks < 8 or banks * INPUT_BYTES < 320 * 1024 * 1024:
        raise ValueError(
            "--banks must provide at least 320 MiB of M4096 input-address rotation (minimum 8)"
        )
...
def run_perf(
    arms: dict[str, Arm], fixtures: list[Fixture], banks: int, samples: int
) -> list[dict[str, Any]]:
    fixtures = [
        fixture for fixture in fixtures if fixture.name.startswith("capture_call")
    ]
...
def gather_and_write(
    ...
    rank_payload = {
        ...
        "fixtures": [
            fixture.provenance | {"name": fixture.name} for fixture in fixtures
        ],
    ...
    if perf_records:
        for left, right in zip(gathered[0]["perf"], gathered[1]["perf"], strict=True):
            ...
            if tuple((left[field] for field in key)) != tuple(
                (right[field] for field in key)
            ):
            ...
            rankmax.append(
                {
                    **{
                        field: left[field]
                        for field in ("round", "order_index", "arm", "bank", "fixture")
                    },
    ...
    summary = {
        ...
        "configuration": {
            ...
            "sum_codec_workspace_min_bytes": MIN_WORKSPACE_BYTES,
            "sum_codec_workspace_requirement": "input.numel()*4 + input.numel()/256*4",
            "sum_codec_header_bytes_per_256_element_block": 4,
        ...
        "changed_input_graph_validation_by_rank": [
            gathered[0]["graph_validation"],
            gathered[1]["graph_validation"],
        ],
        "service_alias_validation_by_rank": [
            gathered[0]["service_alias_validation"],
            gathered[1]["service_alias_validation"],
        ],
        "codec_info_by_rank": [gathered[0]["codec_info"], gathered[1]["codec_info"]],
        "loaded_installed_comm_libraries_by_rank": [
            gathered[0]["loaded_installed_comm_libraries"],
            gathered[1]["loaded_installed_comm_libraries"],
        ],
...
def read_codec_info(namespace: str, sample: torch.Tensor) -> Any:
    info = (
        getattr(torch.ops, namespace).info(sample, 1)
        if namespace == "mach_lossless_prefill_direct"
        else getattr(torch.ops, namespace).info(sample)
    )
    ...
    try:
        ...
        if not library.is_file():
            raise FileNotFoundError(
                f"--library must be an exact existing .so path: {library}"
            )
        reference_library = Path(args.reference_library)
        if not reference_library.is_file():
            raise FileNotFoundError(
                f"--reference-library must be an exact existing .so path: {reference_library}"
            )
        torch.ops.load_library(str(reference_library))
        torch.ops.load_library(str(library))
        for namespace in ("mach_lossless_prefill_sum", "mach_lossless_prefill_direct"):
            if (
                not hasattr(torch.ops, namespace)
                or not hasattr(getattr(torch.ops, namespace), "run")
                or (not hasattr(getattr(torch.ops, namespace), "info"))
            ):
                raise RuntimeError(
                    f"extension did not register torch.ops.{namespace}.run and .info"
                )
        RESIDUAL, GAMMA = make_residual_and_gamma(device)
        fixtures = synthetic_fixtures(rank, device)
        if not args.smoke:
            fixtures.extend(
                (
                    load_capture_fixture(args.capture_dir, rank, call, device)
                    for call in CAPTURE_CALLS
                )
            )
        codec_info = {
            namespace: read_codec_info(namespace, fixtures[0].source)
            for namespace in (
                "mach_lossless_prefill_sum",
                "mach_lossless_prefill_direct",
            )
        }
        arms = make_arms(rank)
...
if __name__ == "__main__":
    try:
        ...
        print(f"bench_direct_codec failed: {error}", file=sys.stderr, flush=True)
        traceback.print_exc()
        raise
