...
# There are two raw-sized payload regions: input and sum. Each has a separate
# 2-byte header per 256 values, totaling 4 header bytes per block.
# The actual FlashInfer metadata value,
# not a guessed allocation, is checked against this complete requirement.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--reference-library", required=True, help="Frozen input+SUM mach_lossless_prefill_sum extension .so path.")
    parser.add_argument("--library", required=True, help="New mach_lossless_prefill_direct extension .so path.")
    ...
    parser.add_argument("--samples", type=int, default=18, help="Interleaved ABCCBA repetitions (36 samples per arm at 18).")
...
def synthetic_fixtures(rank: int, device: torch.device) -> list[Fixture]:
    ...
    # Both rank-local inputs are a single exponent and therefore compact in the
    # input codec; their exact FP32 reduction cancels to zero, exercising a SUM
    # raw-fallback condition without needing NaN/Inf RMSNorm behavior.
    cancellation_value = 1.0 if rank == 0 else -1.0
    cancelling = torch.full((ROWS, HIDDEN), cancellation_value, dtype=torch.bfloat16, device=device)
    # Every 256-value group contains finite BF16 exponents 10..60.  This is a
    # deliberately wide but non-overflowing RMSNorm input (span 50 > 15).
    exponents = torch.arange(10, 61, dtype=torch.int16).repeat((256 + 50) // 51)[:256]
    fractions = torch.arange(256, dtype=torch.int16) & 0x7F
    ...
    return [
        ...
        Fixture(
            ...
            cancelling,
            {"kind": "finite_sum_edge", "rank": rank, "input": cancellation_value, "input_exponent_span": 0,
             "sum_expectation": "exact zero; exercises sum raw fallback"},
        ),
        Fixture(
            "wide_exponent_finite",
            wide_exponent,
            {"kind": "finite_sum_edge", "rank": rank, "bf16_exponent_range": [10, 60], "span": 50,
...
def make_arms(rank: int) -> dict[str, Arm]:
    ...
    # across installed, frozen input+SUM Champion, and new sum-packed paths.
    installed_ws, champion_ws, sum_packed_ws = (create_workspace(rank) for _ in ARM_NAMES)
    ...
    def extension_dispatch(namespace: str, workspace: Any) -> Callable[[torch.Tensor, torch.Tensor, torch.Tensor], None]:
        workspace_bytes = int(workspace.metadata["buffer_size"])
        ...
        def dispatch(input_: torch.Tensor, residual_out: torch.Tensor, norm_out: torch.Tensor) -> None:
            getattr(torch.ops, namespace).run(
                ...
                rank, workspace_bytes, 1e-6, 1.0,
                1 if namespace == 'mach_lossless_prefill_direct' else True, False,
            )
    ...
    return {
        ...
        "champion": Arm("champion", champion_ws, streams["champion"], extension_dispatch("mach_lossless_prefill_sum", champion_ws)),
        "sum_packed": Arm("sum_packed", sum_packed_ws, streams["sum_packed"], extension_dispatch("mach_lossless_prefill_direct", sum_packed_ws)),
    }
...
def validate_fixtures(arms: dict[str, Arm], fixtures: list[Fixture], rank: int) -> list[dict[str, Any]]:
    ...
    for fixture in fixtures:
        ...
        if rank == 0:
            print(f"validated {fixture.name}: installed == champion == sum_packed", flush=True)
    return records
...
def replay_changed_input_graphs(arms: dict[str, Arm], fixtures_by_name: dict[str, Fixture], rank: int) -> list[dict[str, Any]]:
    ...
    for fixture_name in selected:
        ...
        for name in ("champion", "sum_packed"):
            bitwise_equal(graphs["installed"].residual_out, graphs[name].residual_out, f"graph {fixture_name} residual installed vs {name}")
            bitwise_equal(graphs["installed"].norm_out, graphs[name].norm_out, f"graph {fixture_name} norm installed vs {name}")
        records.append({"fixture": fixture_name, "changed_input_graph_replay": True, "bitwise_pipeline_match": True})
        if rank == 0:
            print(f"graph replay validated {fixture_name}: installed == champion == sum_packed", flush=True)
    return records
...
def build_perf_banks(arms: dict[str, Arm], fixtures: list[Fixture], banks: int) -> dict[str, list[GraphBank]]:
    if banks < 8 or banks * INPUT_BYTES < 320 * 1024 * 1024:
        raise ValueError("--banks must provide at least 320 MiB of M4096 input-address rotation (minimum 8)")
...
def run_perf(arms: dict[str, Arm], fixtures: list[Fixture], banks: int, samples: int) -> list[dict[str, Any]]:
    ...
    schedule = ("installed", "champion", "sum_packed", "sum_packed", "champion", "installed")
    records: list[dict[str, Any]] = []
...
def gather_and_write(
    ...
    summary = {
        ...
        "reference_library": {"path": str(Path(args.reference_library).resolve()), "sha256": sha256_file(Path(args.reference_library))},
...
def read_codec_info(namespace: str, sample: torch.Tensor) -> Any:
    info = getattr(torch.ops, namespace).info(sample, 1) if namespace == "mach_lossless_prefill_direct" else getattr(torch.ops, namespace).info(sample)
...
def main() -> None:
    ...
    try:
        ...
        if not reference_library.is_file():
            raise FileNotFoundError(f"--reference-library must be an exact existing .so path: {reference_library}")
        ...
        for namespace in ("mach_lossless_prefill_sum", "mach_lossless_prefill_direct"):
            if not hasattr(torch.ops, namespace) or not hasattr(getattr(torch.ops, namespace), "run") or not hasattr(getattr(torch.ops, namespace), "info"):
                raise RuntimeError(f"extension did not register torch.ops.{namespace}.run and .info")
        ...
        codec_info = {namespace: read_codec_info(namespace, fixtures[0].source) for namespace in ("mach_lossless_prefill_sum", "mach_lossless_prefill_direct")}
