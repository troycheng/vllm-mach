...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--library",
        required=True,
        help="Exact mach_lossless_prefill extension .so path.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=9,
        help="Interleaved ABCCBA schedule repetitions.",
    )
...
def make_arms(rank: int) -> dict[str, Arm]:
    ...
    return {
        ...
        "rebuilt_control": Arm(
            "rebuilt_control",
            rebuilt_ws,
            streams["rebuilt_control"],
            extension_dispatch(False, rebuilt_ws),
        ),
        "packed": Arm(
            "packed", packed_ws, streams["packed"], extension_dispatch(True, packed_ws)
        ),
    }
...
def validate_fixtures(
    ...
    for fixture in fixtures:
        ...
        if rank == 0:
            print(
                f"validated {fixture.name}: installed == rebuilt_control == packed",
                flush=True,
            )
    return records
...
def replay_changed_input_graphs(
    ...
    for fixture_name in selected:
        ...
        bitwise_equal(
            graphs["rebuilt_control"].residual_out,
            graphs["packed"].residual_out,
            f"graph {fixture_name} residual",
        )
        bitwise_equal(
            graphs["rebuilt_control"].norm_out,
            graphs["packed"].norm_out,
            f"graph {fixture_name} norm",
        )
        records.append(
            {
        ...
        if rank == 0:
            print(
                f"graph replay validated {fixture_name}: rebuilt_control == packed",
                flush=True,
            )
    return records
...
def build_perf_banks(
    ...
    if banks < 8 or banks * INPUT_BYTES < 320 * 1024 * 1024:
        raise ValueError(
            "--banks must provide at least 320 MiB of cold M4096 input rotation (minimum 8)"
        )
...
def read_codec_info(sample: torch.Tensor) -> Any:
    ...
    if len(values) != 7:
        raise RuntimeError(
            f"mach_lossless_prefill.info expected 7 values, got {values}"
        )
    names = (
        "packed_regs",
        "packed_local_bytes",
...
def main() -> None:
    ...
    try:
        ...
        if not hasattr(torch.ops, "mach_lossless_prefill") or not hasattr(
            torch.ops.mach_lossless_prefill, "run"
        ):
            raise RuntimeError(
                "extension did not register torch.ops.mach_lossless_prefill.run"
            )
