...
// Derived from FlashInfer fused collective operations; see NOTICE.
// Complete TP2 BF16 AllReduce + residual + Gemma RMSNorm boundary.
// Only the input peer-read leg is compressed. Original FP32 accumulation,
// BF16 conversion, sum publish, barriers and FusedOp are retained.
#include <ATen/ATen.h>
...
namespace mach_lossless_prefill {
...
void run(at::Tensor const& input, at::Tensor const& residual,
  ...
  // Header lives after BOTH raw-sized input and sum payload regions.
  TORCH_CHECK(workspace_bytes >= input.numel() * 4 + input.numel() / 256 * 2);
...
TORCH_LIBRARY(mach_lossless_prefill, m) {
...
TORCH_LIBRARY_IMPL(mach_lossless_prefill, CUDA, m) {
  m.impl("run", &mach_lossless_prefill::run);
  m.impl("info", &mach_lossless_prefill::info);
}
