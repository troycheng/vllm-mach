...
namespace mach_owner_ragged_local {
...
__global__ void ordered_sum_norm_kernel(AllReduceFusionParams<T> params,
  ...
  FusedOp<Pattern, T> fused_op(params,
      access_id < index_helper.tot_access ? access_id : access_id_in_token, access_id_in_token);
...
void ordered_sum_norm(at::Tensor const& part0, at::Tensor const& part1,
  ...
  TORCH_CHECK(part0.dim() == 2 && part0.size(0) >= 1 && part0.size(0) <= kRows && part0.size(1) == kH);
...
TORCH_LIBRARY(mach_owner_ragged_local, m) {
...
TORCH_LIBRARY_IMPL(mach_owner_ragged_local, CUDA, m) {
  m.impl("ordered_sum_norm", &mach_owner_ragged_local::ordered_sum_norm);
}
