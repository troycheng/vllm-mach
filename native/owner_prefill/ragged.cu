...
__global__ void reduce_owner_kernel(AllReduceFusionParams<T> params, int split_rows) {
  ...
  int rows = params.size / params.hidden_dim;
  int own_begin = params.rank == 0 ? 0 : split_rows;
  int own_end = params.rank == 0 ? split_rows : rows;
  int peer_begin = params.rank == 0 ? split_rows : 0;
  int peer_end = params.rank == 0 ? rows : split_rows;
  int own_access_base = own_begin * params.hidden_dim / VEC_SIZE;
  FusedOp<Pattern, T> fused_op(params,
      (token_id < own_end - own_begin ? access_id : access_id_in_token) + own_access_base,
                               access_id_in_token);
  ...
  int peer_tot_access = peer_end * params.hidden_dim / VEC_SIZE;
  ...
  int own_tot_access = own_end * params.hidden_dim / VEC_SIZE;
...
__global__ void gather_mx8_padded_kernel(void** workspace, uint8_t const* local_values,
                                  uint8_t const* local_scales, uint8_t* out_values,
                                  uint8_t* out_scales, int rank, int values_bytes,
                                  int scales_bytes, int valid_values_bytes,
                                  int valid_scales_bytes) {
  ...
  for (int i = int(blockIdx.x * blockDim.x + threadIdx.x); i < packet_vec;
    ...
    if (i < values_vec) {
      uint4 x = i < valid_values_bytes / int(sizeof(uint4))
                    ? local_values4[i] : make_uint4(0, 0, 0, 0);
      out_values4[rank * values_vec + i] = x;
      remote_packet[i] = x;
    } else {
      int j = i - values_vec;
      uint4 x = j < valid_scales_bytes / int(sizeof(uint4))
                    ? local_scales4[j] : make_uint4(0, 0, 0, 0);
...
void reduce_owner(at::Tensor const& input, at::Tensor const& residual,
                  ...
                  at::Tensor& residual_out, at::Tensor& norm_out, int64_t rank, int64_t split_rows,
                  int64_t workspace_bytes, double eps, double weight_bias,
                  bool pdl) {
  TORCH_CHECK(input.is_cuda() && input.dim() == 2 && input.size(0) >= 256 && input.size(0) <= kM &&
              input.size(1) == kH);
  ...
  TORCH_CHECK(split_rows == ((input.size(0) + 255) / 256) * 128 &&
              split_rows > 0 && split_rows < input.size(0),
              "split_rows must be ceil(M/256)*128");
  ...
  TORCH_CHECK(cudaLaunchKernelEx(&config, reduce_owner_kernel, params, int(split_rows)) == cudaSuccess,
              "reduce_owner launch failed");
}

void gather_mx8_padded(at::Tensor const& local_values, at::Tensor const& local_scales,
  ...
  int64_t valid_values_bytes = local_values.numel();
  int64_t valid_scales_bytes = local_scales.numel();
  TORCH_CHECK(out_values.numel() % 32 == 0 && out_scales.numel() % 32 == 0);
  int64_t values_bytes = out_values.numel() / 2, scales_bytes = out_scales.numel() / 2;
  TORCH_CHECK(valid_values_bytes > 0 && valid_scales_bytes > 0 &&
              valid_values_bytes % 16 == 0 && valid_scales_bytes % 16 == 0 &&
              valid_values_bytes <= values_bytes && valid_scales_bytes <= scales_bytes);
  ...
  TORCH_CHECK(cudaDeviceGetAttribute(&sm_count, cudaDevAttrMultiProcessorCount,
              ...
              sm_count > 0,
              "gather_mx8_padded could not query a positive SM count");
  TORCH_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      &active, gather_mx8_padded_kernel, 256, 0) == cudaSuccess && active >= 1,
      "gather_mx8_padded cannot sustain one 256-thread block per SM");
  cudaLaunchConfig_t config{};
  // All CTAs must be resident and have a slot in the barrier workspace.
  config.gridDim = std::min(sm_count, details::kBarrierFlagCount);
  config.blockDim = 256;
  ...
  TORCH_CHECK(cudaLaunchKernelEx(
                  &config, gather_mx8_padded_kernel,
                  ...
                  int(values_bytes), int(scales_bytes),
                  int(valid_values_bytes), int(valid_scales_bytes)) == cudaSuccess,
              "gather_mx8_padded launch failed");
}
}  // namespace mach_owner_ragged

TORCH_LIBRARY(mach_owner_ragged, m) {
  m.def("reduce_owner(Tensor input, Tensor residual, Tensor gamma, Tensor workspace, Tensor(a!) residual_out, Tensor(b!) norm_out, int rank, int split_rows, int workspace_bytes, float eps, float weight_bias, bool pdl) -> ()");
  m.def("gather_mx8_padded(Tensor local_values, Tensor local_scales, Tensor workspace, Tensor(a!) out_values, Tensor(b!) out_scales, int rank, int workspace_bytes, bool pdl) -> ()");
}
TORCH_LIBRARY_IMPL(mach_owner_ragged, CUDA, m) {
  m.impl("reduce_owner", &mach_owner_ragged::reduce_owner);
  m.impl("gather_mx8_padded", &mach_owner_ragged::gather_mx8_padded);
}
