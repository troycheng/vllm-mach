...
// The original M16 implementation is included for shared type/constant
// declarations.  Its function body and instantiations remain unchanged.
#include "quant/exl3_gemm_inner.cuh"

template<EXL3_GEMM_T_ARGS, bool shmem_out_had, bool preserve_half_output = false>
inline __device__
void exl3_gemm_kernel_inner_m32
(
...
)
{
    ...
    (
        TILEBLOCKS_M * 4 * EXL3_GEMM_BASE_THREADS * FRAGS_N_PER_WARP,
    ...
    static_assert(TILESIZE_M == 32, "M32 specialization requires TILESIZE_M == 32");
    ...
    auto write_sum_tile_sh = [&]()
    {
        ...
        for (int m = 0; m < TILEBLOCKS_M; ++m)
        {
            ...
            if (row_base + r0 < size_m)
            {
                const int c = (lane_id % 4) * 2;
                #pragma unroll
                for (int n = 0; n < FRAGS_N_PER_WARP; ++n)
                {
                    float* c_ptr = ((float*) sh_c) + (row_base + r0) * TILESIZE_N + (n0 + n) * 8 + c;
                    *c_ptr++ = frag_c[m][n][0];
                    *c_ptr++ = frag_c[m][n][1];
                }
            }
            if (row_base + r1 < size_m)
            {
                const int c = (lane_id % 4) * 2;
                #pragma unroll
                for (int n = 0; n < FRAGS_N_PER_WARP; ++n)
                {
                    float* c_ptr = ((float*) sh_c) + (row_base + r1) * TILESIZE_N + (n0 + n) * 8 + c;
                    *c_ptr++ = frag_c[m][n][2];
                    *c_ptr++ = frag_c[m][n][3];
                }
            }
