...
ENABLED = os.environ.get('VLLM_MACH_OWNER_PREFILL', '0') == '1'
VERIFY_FORWARDS = int(os.environ.get('VLLM_MACH_OWNER_VERIFY', '0'))
VERIFY_ONLY_RAGGED = os.environ.get('VLLM_MACH_OWNER_VERIFY_ONLY_RAGGED', '0') == '1'
H = 5120


def replica_layers():
    raw = json.loads(os.environ.get('VLLM_MACH_OWNER_MLP_LAYERS', str(list(range(0, 64, 2)))))
    if not isinstance(raw, list) or any(
        type(i) is not int or not 0 <= i < 64 for i in raw
    ) or len(set(raw)) != len(raw):
        raise ValueError('VLLM_MACH_OWNER_MLP_LAYERS must be distinct layer indices in [0, 64)')
    return tuple(raw)
...
def row_partition(rows, rank):
    if not 3000 <= rows <= 4096 or rank not in (0, 1):
        raise ValueError('Owner prefill requires M3000..4096 and TP rank 0 or 1')
    packet_rows = ((rows + 255) // 256) * 128
    counts = (packet_rows, rows - packet_rows)
    own = slice(rank * packet_rows, rank * packet_rows + counts[rank])
    other = slice((1 - rank) * packet_rows,
                  (1 - rank) * packet_rows + counts[1 - rank])
    return packet_rows, counts, own, other


def packed_weight(module, *, merged=True):
    from .mxfp6_hybrid import state_for_rows
    hybrid = state_for_rows(module, 3000)
    if hybrid is None:
        raise RuntimeError('Owner prefill requires the MXFP6 checkpoint profile on every projection')
    if merged:
        weight = hybrid.merged_weight
    else:
        if len(hybrid.weights) != 1:
            raise RuntimeError('Owner down projection requires one row-parallel MXFP6 shard')
        weight = next(iter(hybrid.weights.values()))
    if weight is None:
        raise RuntimeError('Owner prefill requires a loaded merged MXFP6 weight')
    return weight


def load_native():
    from .owner_prefill_native import load
    load()
...
def prepare_mlp_replicas(state):
    ...
    for layer_id in MLP_LAYERS:
        ...
        for key, projection in (('gu', 'gate_up_proj'), ('down', 'down_proj')):
            ...
            assert (original.rows, original.k) == ((17408, H) if key == 'gu' else (H, 8704))
            buffers = {}
            for field in ('values', 'scales'):
                ...
                partner = both.reshape(2, -1)[1-state.rank].clone().reshape(source.shape)
                del both
    ...
    print(json.dumps({'owner_local_mlp_initialized': True, 'rank': state.rank,
        'layers': list(MLP_LAYERS), 'partner_bytes': total}), flush=True)


def local_mlp(state, layer, normalized, label):
    import mxfp6
    from mxfp6.autotune import W6A8Config, _run_config
    ...
    for weights in state.mlp_weights[state.indices[id(layer)]]:
        ...
        y = _run_config(W6A8Config(12, 1, 2), x.values, gu.values, x.scales, gu.scales,
                        state.local_rows, gu.rows, H, torch.bfloat16)
        values = torch.empty((state.local_rows, 8704), dtype=torch.uint8, device=y.device)
        scales = torch.empty(((state.local_rows + 127) // 128 * 128 * 272,), dtype=torch.uint8, device=y.device)
        mxfp6.silu_and_mul_mxfp8_packed_out(values, scales, y)
        partial = _run_config(W6A8Config(14, 1, 1), values, down.values, scales, down.scales,
                             state.local_rows, down.rows, 8704, torch.bfloat16)
    ...
    if state.verify:
        ...
        for rank in (0, 1):
            check(state, label+f'/mlp_tp{rank}', partials[rank], both[rank, state.own])
...
def real_prefill(model):
    ...
    if not isinstance(metadata, dict) or not metadata:
        return False
    first = next(layer.linear_attn for layer in model.layers if layer.layer_type == 'linear_attention')
    # MRV2's autotune dummy runs can have attention metadata but no GDN entry.
    item = metadata.get(first.prefix)
    return item is not None and item.num_prefills > 0


def begin(model, hidden):
    if not ENABLED or hidden.ndim != 2 or hidden.shape[1] != H:
        return None
    m = int(hidden.shape[0])
    if not 3000 <= m <= 4096:
        return None
    # No mid-model fallback: establish one complete model-wide contract first.
    if hidden.dtype != torch.bfloat16 or not hidden.is_contiguous() or model.aux_hidden_state_layers:
        return None
    if hidden.device.type != 'cuda' or torch.cuda.get_device_capability(hidden.device) != (12, 0):
        raise RuntimeError('Owner prefill requires an SM120 CUDA device')
    state = getattr(model, '_owner_prefill_state', None)
    if state is None:
        from vllm.distributed import get_tensor_model_parallel_rank, get_tensor_model_parallel_world_size, get_tp_group
        ...
        for layer in model.layers:
            ...
            assert all(n.weight.dtype == torch.bfloat16 and n.variance_epsilon == 1e-6 for n in norms)
            projection = layer.linear_attn.in_proj_qkvz if layer.layer_type == 'linear_attention' else layer.self_attn.qkv_proj
        ...
        assert model.norm.weight.dtype == torch.bfloat16 and model.norm.variance_epsilon == 1e-6
        load_native()
        ...
        workspace=flashinfer.comm.create_allreduce_fusion_workspace(
            ...
            hidden_dim=H,dtype=torch.bfloat16,group=get_tp_group().cpu_group)
        assert workspace.backend=='trtllm' and int(workspace.metadata['buffer_size'])>=84213760
        state=SimpleNamespace(rank=rank,workspace=workspace,
            ba_weights={},indices={},verified_forwards=0,checks=[],active_checks=[],
            verify=False,total_forwards=0,model=model)
        for i,layer in enumerate(model.layers):
            state.indices[id(layer)]=i
            if layer.layer_type=='linear_attention':
                assert layer.linear_attn.enable_fused_gdn_decode
                layer.linear_attn._fi_stage_decode_parameters()
                weight=layer.linear_attn.in_proj_ba.weight.detach()
                assert weight.dtype==torch.bfloat16 and tuple(weight.shape)==(48,H) and weight.is_contiguous()
        ...
        print(json.dumps({'owner_prefill_initialized':True,'rank':rank,
            'ba_replicated_layers':len(state.ba_weights),'workspace_bytes':int(workspace.metadata['buffer_size']),
            'native_abi':'owner-prefill-v1'}),flush=True)
    assert state.local_mlp_partials is None
    ...
    assert 0 < state.local_rows <= state.p and 0 < state.split_rows[1 - state.rank] <= state.p
    state.verify = state.verified_forwards < VERIFY_FORWARDS and (not VERIFY_ONLY_RAGGED or m != 4096) and real_prefill(model)
    state.active_checks=[]
...
def check(state,label,a,b):
    ...
    same=bool(torch.equal(a.contiguous().view(torch.uint8),b.contiguous().view(torch.uint8)))
    item={'label':label,'shape':list(a.shape),'dtype':str(a.dtype),'bitwise_equal':same}
    if not same:
        d=a.float()-b.float()
...
def gather_bytes(state, a, b, *, scale_atom=False):
    ...
    if state.m == 4096:
        ...
        torch.ops.mach_owner.gather_mx8(a,b,state.workspace.workspace_tensor,out_a,out_b,
            state.rank,int(state.workspace.metadata['buffer_size']),True)
        return out_a,out_b
    ...
    torch.ops.mach_owner_ragged.gather_mx8_padded(
        a,b,state.workspace.workspace_tensor,out_a,out_b,state.rank,
        int(state.workspace.metadata['buffer_size']),True)
...
def norm_step(state,hidden,residual,norm,label,quantize=True,owner_only=False):
    ...
    if state.verify:
        ...
        oracle_norm,oracle_residual=fused_allreduce_gemma_rms_norm(oracle_partial.clone(),full_residual,norm)
    output_norm=torch.empty_like(hidden)
    if pending is None:
        if state.m == 4096:
            torch.ops.mach_owner.reduce_owner(hidden,residual,norm.weight,
                state.workspace.workspace_tensor,hidden,output_norm,state.rank,
                int(state.workspace.metadata['buffer_size']),norm.variance_epsilon,1.0,True)
        else:
            torch.ops.mach_owner_ragged.reduce_owner(hidden,residual,norm.weight,
                state.workspace.workspace_tensor,hidden,output_norm,state.rank,state.p,
                int(state.workspace.metadata['buffer_size']),norm.variance_epsilon,1.0,True)
    else:
        ordered = (torch.ops.mach_owner_local.ordered_sum_norm if state.m == 4096
                   else torch.ops.mach_owner_ragged_local.ordered_sum_norm)
        ordered(pending.parts[0],pending.parts[1],residual[state.own],norm.weight,
                hidden[state.own],output_norm[state.own],norm.variance_epsilon,1.0,True)
    check(state,label+'/residual',hidden[state.own],oracle_residual[state.own] if state.verify else hidden[state.own])
    check(state,label+'/norm',output_norm[state.own],oracle_norm[state.own] if state.verify else output_norm[state.own])
    prepared=None
    if quantize:
        ...
        if owner_only:
            ...
            values,scales=gather_bytes(state,half.values,half.scales,scale_atom=True)
            prepared=mxfp6.MXFP8Tensor(values.view(state.m,H),scales,state.m,H)
        if state.verify:
            reference=mxfp6.quantize_mxfp8(oracle_norm[state.own] if owner_only else oracle_norm)
    ...
    return SimpleNamespace(residual=hidden,norm=output_norm,prepared=prepared,oracle_norm=oracle_norm)


def gdn_forward(state,module,normalized,label):
    from .mxfp6_hybrid import apply_mxfp8_weight
    from vllm.model_executor.layers.mamba.gdn.qwen_gdn_linear_attn import _encode_layer_name
    qkvz=apply_mxfp8_weight(normalized.prepared,packed_weight(module.in_proj_qkvz))
    normalized.norm[state.other].zero_()
    ba0,ba1=(F.linear(normalized.norm,w)[state.own] for w in state.ba_weights[id(module)])
    ...
    core=torch.zeros((state.m,module.num_v_heads//module.tp_size,module.head_v_dim),
                     dtype=torch.bfloat16,device=qkvz.device)
    torch.ops.vllm.qwen_gdn_attention_core_fused_norm_packed(qkvz,ba,core,
...
def attention_forward(state,module,normalized,positions,label):
    ...
    if gate is not None:output=output*torch.sigmoid(gate)
...
def run_layer(state,layer,hidden,residual,positions):
    ...
    if residual is None:
        ...
        residual=n.residual
        hidden=(gdn_forward(state,layer.linear_attn,n,label) if layer.layer_type=='linear_attention'
                else attention_forward(state,layer.self_attn,n,positions,label))
    replicated=state.indices[id(layer)] in state.mlp_weights
    n=norm_step(state,hidden,residual,layer.post_attention_layernorm,label+'/post',owner_only=replicated)
    output=local_mlp(state,layer,n,label) if replicated else layer.mlp(n.prepared)
...
def finish(state,hidden,residual,norm):
    ...
    if state.verify:
        ...
        state.checks.append({'forward':state.verified_forwards,'physical_rows':state.m,
                             'real_prefill':True,'checks':state.active_checks})
        passed=all(c['bitwise_equal'] for f in state.checks for c in f['checks'])
        if not passed:
            raise RuntimeError('Owner prefill shadow verification failed; inspect state.checks')
        print(json.dumps({'owner_prefill_verified':True,'rank':state.rank,
            'verified_forwards':state.verified_forwards,'passed':passed,
            'checks_this_forward':len(state.active_checks)}),flush=True)
    return output
