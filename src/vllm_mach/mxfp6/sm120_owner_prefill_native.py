...
def load():
    ...
    for package, expected in (
        ...
        ("mxfp6-sm120", "0.2.1"),
        ("vllm-mach-owner-prefill", "0.1.0a1"),
    ):
        try:
            version = metadata.version(package).split("+", 1)[0]
        except metadata.PackageNotFoundError as exc:
            raise RuntimeError(f"Owner prefill requires {package}=={expected}") from exc
        if version != expected:
            raise RuntimeError(f"Owner prefill requires {package}=={expected}")
    for name in (
        "mach_owner_prefill_ext",
        "mach_owner_local_ext",
        "mach_owner_ragged_ext",
        "mach_owner_ragged_local_ext",
    ):
        spec = util.find_spec(name)
        if spec is None or spec.origin is None:
            raise RuntimeError(
                "Build and install native/owner_prefill before enabling owner prefill"
            )
        torch.ops.load_library(spec.origin)
    if torch.ops.mach_owner_build.abi() != ABI:
        raise RuntimeError(
            "Owner prefill native ABI mismatch; rebuild native/owner_prefill"
        )
    required = {
    ...
    for namespace, operators in required.items():
        for operator in operators:
            if not hasattr(getattr(torch.ops, namespace), operator):
                raise RuntimeError(
                    f"Owner prefill native operator missing: {namespace}::{operator}"
                )
    _loaded = True
