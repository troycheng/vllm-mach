...
Host side of the ported persistent kernel: one B-dynamic JIT
...
(regular launches only, no allocation). The native adapter in
``vllm_mach.mxfp6.gdn_decode`` gates geometry, dtype and physical rows.
"""
...
from .jit import gen_gdn_fused_decode_module
...
def _geometry_from_tensors(
    ...
    if ssm_state.dtype not in (torch.float16, torch.float32):
        raise ValueError("persistent GDN requires FP16 or FP32 SSM")
    state_bits = 16 if ssm_state.dtype == torch.float16 else 32
    return (hidden, n_ba, qkv_dim, h_q, hv, d, conv_width, conv_state_len, state_bits)
...
def geometry_key(signature: dict) -> tuple:
    ...
    return (
        ...
        int(signature.get("state_bits", 32)),
    )
