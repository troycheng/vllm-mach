...
// State storage is specialized at JIT time; all recurrence arithmetic remains
// FP32. FP16 stores round once per update, without converting the state pool.
#if FI_GDN_STATE_BITS == 16
using State = __half;
struct alignas(8) State4 { __half2 xy; __half2 zw; };
__device__ __forceinline__ float4 load_state4(const State* p) {
  State4 v = *reinterpret_cast<const State4*>(p);
  float2 xy = __half22float2(v.xy), zw = __half22float2(v.zw);
  return make_float4(xy.x, xy.y, zw.x, zw.y);
}
__device__ __forceinline__ void store_state4(State* p, float4 v) {
  *reinterpret_cast<State4*>(p) = {__floats2half2_rn(v.x, v.y),
                                  __floats2half2_rn(v.z, v.w)};
}
#else
static_assert(FI_GDN_STATE_BITS == 32, "Only FP16/FP32 state is supported");
using State = float;
__device__ __forceinline__ float4 load_state4(const State* p) {
...
// index math collapses to compile-time constants and the state rows for
...
//
// Mach adaptation: vLLM 0.29 reserves state index zero as NULL_BLOCK_ID.
// Padded batch rows: a NONPOSITIVE state index (vLLM's PAD_SLOT_ID = -1) marks a
...
__global__ void gdn_fused_decode_kernel(
    ...
    const bf16* __restrict__ dt_bias, const State* ssm_state, const int* __restrict__ state_indices,
    float scale, long state_stride_0, long qkv_stride, long conv_stride_p, long conv_stride_c,
    long conv_stride_t, bf16* __restrict__ output, bf16* updated_conv, State* ssm_out,
  ...
  if (gwarp < warps_needed) {
    ...
    if (idx > 0) {
      ...
      const State* base_srow = ssm_state + pre_row_base + (long)v0 * D;
#pragma unroll
      for (int r = 0; r < ROWS_PER_WARP; ++r) s_pre[r] = load_state4(base_srow + r * D + lane * 4);
    }
  }
...
#pragma unroll
      ...
      const State* base_srow = ssm_state + row_base + (long)v0 * D;
#pragma unroll
      for (int r = 0; r < ROWS_PER_WARP; ++r) s4[r] = load_state4(base_srow + r * D + lane * 4);
    }
...
void gdn_fused_decode_launch(const void* hidden, const void* w_ba, const void* mixed_qkv,
  ...
  if (B == 1) {
    gdn_fused_decode_kernel<true><<<grid, kBlock, 0, stream>>>(
        ...
        static_cast<const State*>(ssm_state), static_cast<const int*>(state_indices), scale,
        state_stride_0, qkv_stride, conv_stride_p, conv_stride_c, conv_stride_t,
        static_cast<bf16*>(output), static_cast<bf16*>(conv_state), static_cast<State*>(ssm_state),
...
void gdn_fused_decode(TensorView hidden_states, TensorView w_ba, TensorView mixed_qkv,
...
#if FI_GDN_STATE_BITS == 16
  CHECK_INPUT_TYPE(ssm_state, dl_float16);
#else
  ...
  //  - NONPOSITIVE (NULL_BLOCK_ID = 0 or PAD_SLOT_ID = -1) is the padding convention. The
  ...
  // Dense inner layout everywhere; the state pool may have a padded row
  ...
  // Four-element vector accesses require page strides divisible by four
  // (16-byte FP32 or 8-byte FP16 alignment). D*D and v*D
  ...
  TVM_FFI_ICHECK_EQ(ssm_state.stride(0) % 4, 0)
      << "ssm_state page stride must be a multiple of 4 state elements";
