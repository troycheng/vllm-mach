...
def _resolve_gdn_fused_decode_step() -> tuple[object, object] | None:
    """Resolve the optional legacy FlashInfer SM120 fused decode API."""
    if os.environ.get("VLLM_ENABLE_QWEN_GDN_FUSED_DECODE", "0") != "1":
        return None
    ...
    try:
        ...
        if step is None or supported is None:
            ...
            step = gdn_fused_decode_step
            supported = gdn_fused_decode_step_supported
        if step is None or supported is None:
            return None
        parameters = inspect.signature(supported).parameters.values()
    except (ImportError, TypeError, ValueError, AttributeError):
        return None
    if not any(
        p.name == "conv_state_layout" or p.kind is inspect.Parameter.VAR_KEYWORD
        for p in parameters
    ):
        return None
...
class QwenGatedDeltaNetAttention(GatedDeltaNetAttention):
    ...
    def __init__(
        ...
        self._fi_fused_decode_supported: object | None = None
        ...
        if not self.gqa_interleaved_layout and self.tp_size == 2:
            resolved = _resolve_gdn_fused_decode_step()
            if resolved is not None:
                self._fi_fused_decode_step, self._fi_fused_decode_supported = resolved
    ...
    def _fi_stage_decode_parameters(self) -> bool:
        """Stage immutable parameters before a CUDA-graph capture."""
        if self._fi_setup_failed:
            return False
        if self._fi_w_ba is not None:
            return True
        weight = getattr(self.in_proj_ba, "weight", None)
        ...
        if not (
            ...
            and conv_weight.dtype == torch.bfloat16
            ...
            return False
        self._fi_w_ba = weight.detach().t().contiguous()
        self._fi_conv_weight = conv_weight.detach().view(
            conv_weight.size(0), conv_weight.size(2)
        ).contiguous()
        self._fi_conv_bias = conv_bias.detach() if conv_bias is not None else torch.zeros(
            self._fi_conv_weight.size(0), dtype=torch.bfloat16,
    ...
    def _forward_core_fi(
        ...
        """Run legacy FlashInfer only for Qwen3.8 TP2 non-spec decode."""
        raw = get_forward_context().attn_metadata
        if (
            self._fi_fused_decode_step is None
            or self._fi_fused_decode_supported is None
            or not self.enable_packed_recurrent_decode
            or raw is None
            or not hasattr(self, "kv_cache")
            or hidden_states.dtype != torch.bfloat16
            or self.kv_cache[0].dtype != torch.bfloat16
            or self.kv_cache[1].dtype != torch.float32
            or torch.cuda.is_current_stream_capturing() and self._fi_w_ba is None
        ):
            return False
        if not isinstance(raw, dict):
            return False
        metadata = raw.get(self.prefix)
        if not isinstance(metadata, GDNAttentionMetadata):
            return False
        if metadata.spec_sequence_masks is not None or metadata.num_prefills != 0:
            return False
        if metadata.num_decodes <= 0:
            return False
        conv_dim_first = is_conv_state_dim_first()
        supported = self._fi_fused_decode_supported(
        ...
        if not supported:
            return False
        if self._fi_w_ba is None and not self._fi_stage_decode_parameters():
            return False
        state_indices = metadata.non_spec_state_indices_tensor
        conv_state = self.kv_cache[0] if conv_dim_first else self.kv_cache[0].transpose(-1, -2)
        self._fi_fused_decode_step(
            hidden_states=hidden_states[: metadata.num_actual_tokens],
            w_ba=self._fi_w_ba,
            mixed_qkv=mixed_qkv[: metadata.num_actual_tokens],
            ...
            state_indices=state_indices[: metadata.num_actual_tokens],
            use_qk_l2norm=True,
            out=core_attn_out[: metadata.num_actual_tokens].unsqueeze(1),
        )
        return True
    ...
    def forward_cuda(
        ...
        if use_fused_gdn_decode:
            ...
            if (
                ...
                and self._fi_fused_decode_step is not None
            ):
                # The optional SM120 kernel owns the B/A projection when its
                # exact decode geometry is registered. The custom op keeps the
                # existing v0.28 fused path as the unsupported-shape fallback.
...
def qwen_gdn_attention_core_fi(
    ...
    """Use the optional SM120 decode kernel with the v0.28 fused fallback."""
    layer_name = _resolve_layer_name(layer_name)
    forward_context: ForwardContext = get_forward_context()
    self = forward_context.no_compile_layers[layer_name]

    # Profile/warmup runs stage immutable parameters before CUDA Graph capture.
    if self._fi_w_ba is None and not torch.cuda.is_current_stream_capturing():
        self._fi_stage_decode_parameters()

    qkv_size = (self.key_dim * 2 + self.value_dim) // self.tp_size
    mixed_qkv, output_gate_flat = mixed_qkvz.split(
        [qkv_size, self.value_dim // self.tp_size], dim=-1
    )
    if self._forward_core_fi(mixed_qkv, hidden_states, core_attn_out):
        attn_metadata_raw = forward_context.attn_metadata
