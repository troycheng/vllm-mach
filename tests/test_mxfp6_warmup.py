...
def test_warmup_prepares_gdn_and_dense_producers_before_workspace(monkeypatch) -> None:
    from vllm_mach.mxfp6 import ar_norm, fused_mlp, gdn_decode, moe_projection
    ...
    monkeypatch.setattr(
        moe_projection,
        "prepare",
        lambda model, problems, dtype: calls.append("projection"),
    )
    monkeypatch.setattr(warmup, "_import_mxfp6", lambda: Runtime)
    monkeypatch.setattr(warmup.torch.cuda, "synchronize", lambda device: None)
    ...
    assert calls == ["gdn", "mlp", "ar_norm", "projection", "begin", "warm", "finalize"]
