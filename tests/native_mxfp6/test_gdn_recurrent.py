"""Exact value-tile recurrence across auxiliary-stream graph replays."""
...
@pytest.mark.parametrize('rows,dtype,layout', list(product(
    (16, 24, 32), (torch.float32, torch.float16), ('SD', 'DS'))))
@pytest.mark.parametrize('candidate_copies', (True, False))
@torch.inference_mode()
def test_recurrent_tile_graph(rows, dtype, layout, candidate_copies):
    ...
    from vllm_mach.mxfp6.gdn.recurrent import packed_decode
    ...
    def call(index, copy):
        ...
        fn = packed_decode if index == 0 else recurrent
        fn(mixed_qkv=cq, a=a, b=b, A_log=al, dt_bias=dt,
            scale=128**-0.5, initial_state=states[index], out=outputs[index],
            ssm_state_indices=indices, use_qk_l2norm_in_kernel=True)

    for i in range(3):
        call(i, candidate_copies if i == 0 else i == 1)
