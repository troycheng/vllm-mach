...
The runtime includes the tested EXL3 Dense path and an alpha MXFP6 Dense bridge. The MXFP6 bridge accepts only static E3M2 weights with dynamic MXFP8 E4M3 activations on SM120. The version-locked runtime profile supplies Stream-K workspace and graph-capture-stream warmup. It does not include checkpoint conversion, routed MoE, or GDN integration. The full public `mxfp6_sm120` vLLM example remains the reference for those additional patches.
