...
CHUNK = 128
PARTIAL_WAVES = 23
EXPECTED_NEGATIVE_OFFSETS = tuple(CHUNK * wave - PROMPT_LEN_SUBMITTED
                                  for wave in range(1, PARTIAL_WAVES + 1))
EXPECTED_SAMPLER_SHA256 = "408b08c4ac66de271f476460490f360ef2a9b73528a346d132a22326e1674369"
EXPECTED_INPUT_BATCH_SHA256 = "3929c92e42ae90e4410bb4537dcbdcd171a662e44afc1189a9a3e19037a84410"
...
def _offsets(sampler, idx_mapping_np, positions):
    ...
    return [int(position) - int(table[int(index)]["prompt_len"]) + 1
...
def _patch_sampler_class(Sampler):
    ...
    _require(hasattr(Sampler, "_k4o_teacher_originals"),
             "install frozen teacher_decode_hook before chunked bridge")
    _require(not hasattr(Sampler, "_k4o_chunked_teacher_bridge_originals"),
    ...
    def sample(self, logits, expanded_idx_mapping, idx_mapping, idx_mapping_np,
               pos, input_ids, expanded_local_pos, *, return_logprobs):
        table, active = _teacher_rows(self, idx_mapping_np)
        if not any(active):
            return teacher_sample(self, logits, expanded_idx_mapping, idx_mapping,
                                  idx_mapping_np, pos, input_ids, expanded_local_pos,
                                  return_logprobs=return_logprobs)
        _require(all(active) and len(active) == ROWS and int(logits.shape[0]) == ROWS,
                 f"mixed/non-M32 teacher batch: active={active} logits={tuple(logits.shape)}")
        import torch
        _require(not torch.cuda.is_current_stream_capturing(),
        ...
        unique = set(offsets)
        _require(len(unique) == 1, f"ready/partial teacher rows mixed: offsets={offsets}")
        offset = offsets[0]
        if offset >= 0:
            return teacher_sample(self, logits, expanded_idx_mapping, idx_mapping,
                                  idx_mapping_np, pos, input_ids, expanded_local_pos,
                                  return_logprobs=return_logprobs)
        _require(offset in EXPECTED_NEGATIVE_OFFSETS,
                 f"unexpected partial-prefill offset {offset}")
        _require(getattr(self, "_k4o_chunked_partial_pending", None) is None,
        ...
        return native_sample(self, logits, expanded_idx_mapping, idx_mapping,
                             idx_mapping_np, pos, input_ids, expanded_local_pos,
                             return_logprobs=return_logprobs)
    ...
    def call(self, logits, input_batch):
        ...
        if pending is None:
            return result
        _require(int(input_batch.num_reqs) == ROWS and int(input_batch.num_tokens) == 4096,
        ...
        _require(scheduled == [CHUNK] * ROWS,
                 f"partial wave is not 32x128: {scheduled}")
        prefilling = [bool(value) for value in input_batch.is_prefilling_np.tolist()]
        _require(prefilling == [True] * ROWS,
                 f"partial wave contains ready request: {prefilling}")
        mapped = [self._k4o_teacher[int(index)]["id"] for index in input_batch.idx_mapping_np]
        _require(mapped == pending["ids"] and len(set(mapped)) == ROWS,
                 "partial-wave teacher request mapping changed or aliases")
        num_sampled = [int(value) for value in result.num_sampled.detach().cpu().tolist()]
        num_rejected = [int(value) for value in result.num_rejected.detach().cpu().tolist()]
        _require(num_sampled == [0] * ROWS and num_rejected == [0] * ROWS,
...
def install(worker):
    ...
    _require(_sha(sampler_path) == EXPECTED_SAMPLER_SHA256,
             f"Sampler source mismatch: {sampler_path}")
    _require(_sha(input_path) == EXPECTED_INPUT_BATCH_SHA256,
             f"InputBatch source mismatch: {input_path}")
    _patch_sampler_class(Sampler)
    sampler = worker.model_runner.sampler
    _require(isinstance(sampler, Sampler) and getattr(worker, "_k4o_teacher_sampler", None) is sampler,
    ...
    return {
        ...
        "sampler_source": {"path": str(sampler_path), "sha256": _sha(sampler_path)},
        "input_batch_source": {"path": str(input_path), "sha256": _sha(input_path)},
...
def observations(worker):
    ...
    _require(offsets == list(EXPECTED_NEGATIVE_OFFSETS),
             f"partial-prefill wave sequence mismatch: {offsets}")
    expected_ids = set(rows[0]["ids"]) if rows else set()
    _require(len(expected_ids) == ROWS
             and all(set(item["ids"]) == expected_ids for item in rows),
             "partial-prefill request set changed")
    return {"rank": int(get_tensor_model_parallel_rank()), "calls": rows,
            "partial_wave_count": len(rows), "offsets": offsets}
