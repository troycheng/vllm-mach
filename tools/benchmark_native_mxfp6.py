#!/usr/bin/env python3
"""Deterministic OpenAI-completions benchmark with per-request results."""
...
async def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        ...
        if warmups and not all(
            item["success"] for item in await asyncio.gather(*warmups)
        ):
    ...
    return {
        ...
        "model": args.model,
        "contract": contract,
        "benchmark_started_epoch": time.time()
        - (time.perf_counter() - benchmark_start),
        "aggregate": aggregate,
        "requests": results,
        "warmup": {
            "requests": min(args.warmup_requests, len(prompts)),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--request-rate", type=float, default=0, help="0 means simultaneous arrival"
    )
